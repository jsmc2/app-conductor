...
import { NoticeMgtModule } from '../model/noticeMgt'
...
const withFacingDownApex = (WrappableComponent) => {
  ...
  const { notify, addNoticeMgtDependencies } = NoticeMgtModule()
  ...
  const toolsReducer = (state = {history, notify}) => { return state }

  const topLevelStore = createStore(
    getRootReducer(toolsReducer),
    composeWithDevTools()
  )
  void addNoticeMgtDependencies(noticeHandlers, topLevelStore.dispatch, topLevelStore.getState)
